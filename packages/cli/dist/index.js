...
import { existsSync as existsSync2, mkdirSync as mkdirSync2, unlinkSync, writeFileSync as writeFileSync2 } from "fs";
...
function read() {
  ...
  try {
    return JSON.parse(readFileSync3(SETTINGS_PATH, "utf-8"));
  } catch {
    return null;
  }
}
...
function autoCommand(off) {
  ...
  if (existsSync4(LOG_PATH)) {
    const lines = readFileSync4(LOG_PATH, "utf-8").trimEnd().split("\n").slice(-6);
    console.log("\nLast run:");
...
Linked ${deviceName} to ${redeemed.username ?? "your account"}.`);
  ...
  const scheduler = isInstalled() ? null : install();
