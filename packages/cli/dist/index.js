...
var SCRIPT_PATH = join3(CONFIG_DIR, "auto-push.sh");
var LOG_PATH = join3(CONFIG_DIR, "auto-push.log");
var PLIST_PATH = join3(homedir3(), "Library", "LaunchAgents", "com.tokenmax.push.plist");
var CRON_TAG = "# tokenmax auto-push";
var LABEL = "com.tokenmax.push";
var HOURS = [0, 6, 12, 18];
var SCHEDULE_LABEL = "every 6 hours";
function schedulerFor(platform = process.platform) {
...
function writeScript() {
  if (!existsSync2(CONFIG_DIR)) mkdirSync2(CONFIG_DIR, { recursive: true, mode: 448 });
  writeFileSync2(SCRIPT_PATH, script(), { encoding: "utf-8", mode: 448 });
}
function npxPath(nodeDir) {
  const candidate = join3(nodeDir, "npx-cli.js");
  return existsSync2(candidate) ? candidate : join3(nodeDir, "npx");
}
function plist() {
...
function isInstalled() {
  ...
  if (scheduler === "launchd") return existsSync2(PLIST_PATH);
...
function install() {
  ...
  if (scheduler === "launchd") {
    mkdirSync2(join3(homedir3(), "Library", "LaunchAgents"), { recursive: true });
    writeFileSync2(PLIST_PATH, plist(), "utf-8");
    try {
...
function uninstall() {
  const scheduler = schedulerFor();
  if (scheduler === "launchd" && existsSync2(PLIST_PATH)) {
    try {
      execFileSync("launchctl", ["unload", PLIST_PATH], { stdio: "ignore" });
    } catch {
    }
  ...
  if (existsSync2(SCRIPT_PATH)) unlinkSync(SCRIPT_PATH);
}
...
var SETTINGS_PATH = join4(homedir4(), ".claude", "settings.json");
var TIMEOUT_SECONDS = 120;
function isOurs(group) {
  return group.hooks?.some((h) => h.command?.includes(SCRIPT_PATH)) ?? false;
}
function read() {
  if (!existsSync3(SETTINGS_PATH)) return {};
  try {
    return JSON.parse(readFileSync4(SETTINGS_PATH, "utf-8"));
  } catch {
    return null;
  }
}
function write(settings) {
  mkdirSync3(dirname2(SETTINGS_PATH), { recursive: true });
...
function autoCommand(off) {
  ...
  if (existsSync4(LOG_PATH)) {
    const lines = readFileSync5(LOG_PATH, "utf-8").trimEnd().split("\n").slice(-6);
    console.log("\nLast run:");
...
function parseArgs(argv) {
  const options = { dryRun: false, off: false, apiUrl: DEFAULT_API_URL, help: false };
  let command;
  let arg;
