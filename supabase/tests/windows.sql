...
insert into public.profiles (id, username, role) values (:'uid', 'windows-test', 'member');
...
select pg_temp.expect('today = tomorrow + today',        cost_1d,  2) from public.leaderboard;
select pg_temp.expect('7d spans today-6 .. tomorrow',    cost_7d,  4) from public.leaderboard;
select pg_temp.expect('30d spans today-29 .. tomorrow',  cost_30d, 6) from public.leaderboard;
...
select pg_temp.expect('active days in 30d', active_days_30d, 6) from public.leaderboard;
