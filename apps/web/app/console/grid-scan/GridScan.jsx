/* LOCAL EDIT, the one deviation from the react-bits registry copy.
 * Upstream imports face-api.js at module scope and loads its model weights
 * from cdn.jsdelivr.net in an effect with no guard, so every view of this page
 * pulled several MB of face-detection weights from a third party and shipped
 * ~700KB of the library in the bundle, for a feature that only runs when
 * enableWebcam is true. Nothing here passes enableWebcam. It is now imported
 * on demand, so the default path costs nothing and the webcam path behaves
 * exactly as upstream. Re-apply this if the component is ever re-pulled. */
let faceapi = null;
